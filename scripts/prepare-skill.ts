 ...
 * whole document. So it is an edit somebody makes, and `checkSkillFences` in
 * scripts/checks/skill-fences.ts is what stops a re-vendor dropping it in silence: re-vendor
 * one of those two and the check fails until the fence is raised again.
 *
 * Frontmatter is edited a line at a time rather than parsed and re-emitted, unlike
 * `frontmatter` in scripts/checks/support.ts, which parses it. A YAML round-trip reformats
 * every key this script does not touch and drops the comments some skills carry.
 *
