...
export const STANDING_DETAIL: ReadonlyMap<string, string> = new Map([
    ...
    ["copilot-web-design-reviewer", "No page was rendered, so element overflow and overlap, alignment and computed spacing, the mobile, tablet, desktop and wide viewport sweep, whether the resulting keyboard tab order is logical, whether text visibly clips or its ellipsis renders, focus-indicator visibility, hover and active states, and layout shift on load were not evaluated. A contrast ratio or a target size is reported only where the diff, or a file read alongside it, settles the colour pair or the box outright; neither was reached otherwise."],
