 ...
 *
 * Bun's fetch honours `file:`, so a `Location` naming one would have the download read a local
 * path and hand the bytes on as an artifact. The scheme was the whole of the check: every
 * https host was followed and whatever came back was parsed into this run's previous.json,
 * which decides which findings are marked `already-reported`. A suppressed finding is one
 * nobody sees. Everything else on this path is qualified by `sameWorkflow`,
 * `fromThisRepository` and `postedFor`, and the bytes those checks qualify came through the
 * one hop nothing qualified.
 *
 * What bounds the destination is still that api.github.com emits the `Location` in an
 * authenticated TLS response, and that is the one input the fetch takes on faith. The host
 ...
 * direction that whole path already fails in.
 */
export function storageUrl(location: string): string {
